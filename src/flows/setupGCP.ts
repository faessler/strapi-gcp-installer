...
export const setupGCP = async () => {
  ...
  try {
    // project
    cliSync(`gcloud projects create ${PROJECT_ID}`);
    PROJECT_NUMBER = cliSync(`gcloud projects describe ${PROJECT_ID}`).match(
      /(?<=projectNumber: ')[^\s]+(?=')/gm,
    )[0];

    // billing
    cliSync(`gcloud beta billing projects link ${PROJECT_ID} --billing-account ${BILLING_ACCOUNT}`);
    await wait();

    // services
    cliSync(`gcloud services enable iamcredentials.googleapis.com appengine.googleapis.com cloudbuild.googleapis.com --project="${PROJECT_ID}"`);

    // app engine
    cliSync(`gcloud app create --project="${PROJECT_ID}" --region="${REGION}"`);
    ...
    if (!!REPOSITORY) {
      ...
      const permissions = [
        ...
        .join(",");
      await wait();
      cliSync(`gcloud iam roles create ${GITHUB_ROLE_ID} --project="${PROJECT_ID}" --title="roles/github-actions" --permissions="${permissions}" --quiet`);
      await wait();
      
      // service account for github (with previously created role and access to workload identity pool)
      cliSync(`gcloud iam service-accounts create ${GITHUB_SA} --display-name="GitHub Actions SA" --project="${PROJECT_ID}"`);
      cliSync(`gcloud projects add-iam-policy-binding ${PROJECT_ID} --member="serviceAccount:${GITHUB_SA}@${PROJECT_ID}.iam.gserviceaccount.com" --role="projects/${PROJECT_ID}/roles/${GITHUB_ROLE_ID}" --project="${PROJECT_ID}"`);
      cliSync(`gcloud iam service-accounts add-iam-policy-binding ${GITHUB_SA}@${PROJECT_ID}.iam.gserviceaccount.com --role="roles/iam.workloadIdentityUser" --member="principalSet://iam.googleapis.com/projects/${PROJECT_NUMBER}/locations/global/workloadIdentityPools/${GITHUB_WIF_POOL}/*" --project="${PROJECT_ID}"`);
